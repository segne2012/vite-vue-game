<template>
    <div class="cssBtn">
        <div class="menuItem" :class="{ 'active': getActive(item.name) }" v-for="item in list" :key="item.name"
            @click="routerPush(item.path)">
            <div class="iconList">
                <component class="iconItem" :is="item.iconComp"></component>
            </div>
            <span>{{ item.name }} </span>
        </div>
    </div>
